...
const mapAttendanceToDTR = (attendance_records = []) => {
  console.log(
    "[DTRforEmployee] mapping attendance records count:",
...
function printPayslipsToWindow({ payrolls = [], hideValues = true }) {
  ...
  payrolls.forEach((p) => {
    ...
    const dateUntilDay =
      dateUntilParts.length === 3 ? Number(dateUntilParts[2]) : 0;
    ...
    const holidayCountRaw =
      Number(
        p.holiday_count ?? (Array.isArray(p.holidays) ? p.holidays.length : 0),
      ) || 0;
    ...
    if (p.total_allowances !== undefined && p.total_allowances !== null) {
      ...
      allowanceTotal = p.allowances.reduce(
        (s, a) => s + (parseFloat(a.applied_amount) || 0),
        0,
      );
    }
    ...
    if (typeof p.total_rewards !== "undefined" && p.total_rewards !== null) {
      ...
      rewardTotal = p.rewards.reduce(
        (s, r) => s + (parseFloat(r.amount ?? r.applied_amount ?? 0) || 0),
        0,
      );
    }
    ...
    const loan =
      parseFloat(
        p.loan_deduction_applied ??
          p.loan_deduction_actual ??
          p.loan_deduction ??
          0,
      ) || 0;
    ...
    const grossWithExtras =
      baseGross + allowanceTotal + incentiveAmount + retroApplied + rewardTotal;
...
const DTRforEmployee = () => {
  ...
  React.useEffect(() => {
    ...
    if (initialPayroll) {
      console.log(
        "[DTRforEmployee] initial payroll provided",
        initialPayroll.payroll_id,
      );
    ...
    if (!employeeData?.employee_id) {
      console.error(
        "[DTRforEmployee] missing employeeData in navigation state",
      );
      setLoading(false);
      return;
    }
    ...
    const fetchPayrolls = async () => {
      try {
        console.log(
          "[DTRforEmployee] calling fetchPayrollsAPI with",
        ...
        console.log(
          "[DTRforEmployee] raw fetchPayrollsAPI response:",
          response,
        );
        ...
        if (Array.isArray(response)) {
          ...
          console.warn(
            "[DTRforEmployee] Unexpected response shape from fetchPayrollsAPI — could not extract payroll array. Response keys:",
        ...
        console.log(
          "[DTRforEmployee] extracted total payrolls from API:",
          allPayrolls.length,
        );

        // Filter payrolls that match THIS employee_id
        const employeePayrolls = allPayrolls.filter(
          (p) => String(p.employee_id) === String(employeeData.employee_id),
        );
        console.log(
          "[DTRforEmployee] payrolls matching employee_id:",
          employeePayrolls.length,
        );

        if (employeePayrolls.length > 0) {
          setPayrollList(employeePayrolls);
          // choose the latest payroll by date_until
          const sorted = employeePayrolls.sort(
            (a, b) => new Date(b.date_until) - new Date(a.date_until),
          );
          const latest = sorted[0];
          console.log(
            "[DTRforEmployee] selected latest payroll for employee",
            latest.payroll_id,
            latest.date_from,
            latest.date_until,
          );
          ...
          console.warn(
            "[DTRforEmployee] NO payrolls found for employee in API response. Check backend filter by employee_id or response shape.",
          );
          setPayrollList([]);
        }
      } catch (err) {
  ...
  const applyPayroll = (payroll) => {
    ...
    console.log(
      "[DTRforEmployee] applyPayroll attendance count:",
      records.length ?? 0,
    );
  ...
  const isProjectBased = (payroll) => {
    const employeeTypeNormalized = String(
  ...
  const PayslipCard = ({ payroll }) => {
    ...
    let allowanceTotal = 0;
    if (
      payroll.total_allowances !== undefined &&
      payroll.total_allowances !== null
    ) {
      allowanceTotal = parseFloat(payroll.total_allowances) || 0;
    } else if (Array.isArray(payroll.allowances)) {
      allowanceTotal = payroll.allowances.reduce(
        (s, a) => s + (parseFloat(a.applied_amount) || 0),
        0,
      );
    }
    ...
    let rewardTotal = 0;
    if (
      typeof payroll.total_rewards !== "undefined" &&
      payroll.total_rewards !== null
    ) {
      rewardTotal = parseFloat(payroll.total_rewards) || 0;
    } else if (Array.isArray(payroll.rewards)) {
      rewardTotal = payroll.rewards.reduce(
        (s, r) => s + (parseFloat(r.amount ?? r.applied_amount ?? 0) || 0),
        0,
      );
    }
    ...
    if (Array.isArray(payroll.rewards) && payroll.rewards.length > 0) {
      const titles = payroll.rewards
        .map((r) =>
          (r.description || r.name || r.title || "").toString().trim(),
        )
        .filter(Boolean);
    ...
    const loan =
      parseFloat(
        payroll.loan_deduction_applied ??
          payroll.loan_deduction_actual ??
    ...
    if (Array.isArray(payroll.allowances) && payroll.allowances.length > 0) {
      payroll.allowances.forEach((a) => {
        allowanceRows.push({
          label: a.allowance_name || a.name || "Allowance",
          value: show(parseFloat(a.applied_amount) || 0, hideValues),
        });
      });
    } else {
      ...
      allowanceRows.push({
        label: "Allowances",
        value: show(allowanceTotal, hideValues),
      });
    }
    ...
    const finalRows = [
      ...
      {
        label: "Gross (incl. ALL/INC/REW/RETRO)",
        value: show(gross, hideValues),
        strong: true,
      },
        ...
        <CardContent>
          <Grid container alignItems="center" spacing={1}>
            <Grid item xs={8}>
              ...
              <Typography
                ...
                sx={{ display: "block", fontSize: isMobile ? 12 : 13 }}
              >
                {payroll.position_name} • {formatDate(payroll.date_from)} →{" "}
          ...
          <Box>
            {finalRows.map((r, idx) => (
              ...
              >
                <Typography
                  variant={r.strong ? "subtitle2" : "body2"}
                  color={r.strong ? "text.primary" : "text.secondary"}
                >
                  {r.label}
                </Typography>
                <Typography
                  variant={r.strong ? "subtitle2" : "body2"}
                  sx={{ fontWeight: r.strong ? 700 : 500 }}
                >
                  {r.value}
                </Typography>
              </Box>
          ...
          <Stack direction={isMobile ? "column" : "row"} spacing={1}>
            <Button
              ...
              startIcon={<PrintIcon />}
              onClick={() =>
                printPayslipsToWindow({ payrolls: [payroll], hideValues })
              }
            >
              Print Payslip
            </Button>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            const payroll = payrollList.find(
              (p) => String(p.payroll_id) === String(pid),
            );
            if (payroll) applyPayroll(payroll);
            else
              console.warn(
                "[DTRforEmployee] selected payroll id not found",
                pid,
              );
          }}
          style={{
        ...
        >
          {payrollList.length === 0 && selectedPayroll && (
            <option value={selectedPayroll.payroll_id}>
              {formatDate(selectedPayroll.date_from)} →{" "}
              {formatDate(selectedPayroll.date_until)}
            </option>
          )}
          {payrollList.map((p) => (
      ...
      <Box sx={{ mt: 2 }}>
        ...
        >
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Tooltip
              title={hideValues ? "Values are hidden" : "Values are visible"}
            >
              <IconButton
                onClick={() => setHideValues((prev) => !prev)}
                color={hideValues ? "default" : "primary"}
              >
            ...
            <Button
              ...
              onClick={() =>
                printPayslipsToWindow({
                  payrolls: selectedPayroll ? [selectedPayroll] : [],
                  hideValues,
                })
              }
            >
              ...
              {payrollList.map((p) => (
                <Chip
                  ...
                  onClick={() => {
                    ...
                    setExpanded(true);
                    setTimeout(() => {
                      const el = document.querySelector(".MuiAccordion-root");
                      if (el)
                        el.scrollIntoView({
                          behavior: "smooth",
                          block: "center",
                  ...
                  color={
                    selectedPayroll?.payroll_id === p.payroll_id
                      ? "primary"
                      : "default"
                  }
                  variant={
